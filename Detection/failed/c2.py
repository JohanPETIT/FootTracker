...
# Randomly select 500 batches
random.seed(42)
selected_batches = random.sample(list(zip(batches, labels)), 500)

# Prepare the dataset for oversampling
X = [[batch] for batch, _ in selected_batches]  # Convertion to 2D array
y = [label for _, label in selected_batches]

# Perform random oversampling
ros = RandomOverSampler(random_state=42)
X_resampled, y_resampled = ros.fit_resample(X, y)

# Reconstruct the selected_batches with resampled data
selected_batches_resampled = list(zip(X_resampled, y_resampled))

# Split the resampled batches into train and test sets with ratio 70% and 30%
train_size = int(0.7 * len(selected_batches_resampled))
train_batches = selected_batches_resampled[:train_size]
test_batches = selected_batches_resampled[train_size:]
